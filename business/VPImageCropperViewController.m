...
@property(nonatomic,assign) CGSize tiatioFrame;
...
- (id)initWithImage:(UIImage *)originalImage cropFrame:(CGRect)cropFrame limitScaleRatio:(NSInteger)limitRatio Ratio:(CGSize)tatio isError:(NSString*)isError {
    self = [super init];
    if (self) {
        self.cropFrame = cropFrame;
        self.limitRatio = limitRatio;
        self.originalImage = originalImage;
        self.tiatioFrame=tatio;
        IsError=isError;
    }
    return self;
}
...
////    CGFloat oriHeight = self.cropFrame.size.height;//self.originalImage.size.height * (oriWidth / self.originalImage.size.width);
...
//    self.showImgView.frame = self.oldFrame;
    ...
    CGFloat width=self.tiatioFrame.width;
    CGFloat hight=self.tiatioFrame.height;
    riatioBool=true;
    if (width<=hight) {
        riation=width/hight;
        self.cropFrame=CGRectMake(self.cropFrame.origin.x,self.cropFrame.origin.y,self.cropFrame.size.width*riation,self.cropFrame.size.height);
    }
    else
        {
        riatioBool=false;
        riation=hight/width;
        self.cropFrame=CGRectMake(self.cropFrame.origin.x,self.cropFrame.origin.y,self.cropFrame.size.width,self.cropFrame.size.height*riation);
        }
    
       [self addGestureRecognizers];
    if (self.cropFrame.size.width<=self.view.frame.size.width) {
        CGFloat CropX=(self.view.frame.size.width-self.cropFrame.size.width)/2;
        self.cropFrame=CGRectMake(CropX,self.cropFrame.origin.y,self.cropFrame.size.width,self.cropFrame.size.height);
    }
    if (self.cropFrame.size.height<self.view.frame.size.width) {
        
        CGFloat CropY=(self.view.frame.size.width-self.cropFrame.size.height)/2;
        self.cropFrame=CGRectMake(self.cropFrame.origin.x,CropY+self.cropFrame.origin.y,self.cropFrame.size.width,self.cropFrame.size.height);
...
- (void)cancel:(id)sender {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"reSizeImage" object:nil];
...
- (void)confirm:(id)sender {
            ...
            //通过代理回传给上一个界面显示
        [self.Passdelegate passImage:[self getSubImage] andSize:CGSizeMake(self.tiatioFrame.width,self.tiatioFrame.height)];
        [[NSNotificationCenter defaultCenter] postNotificationName:@"reSizeImage" object:nil];
...
//    if (riatioBool) {
//    if ([IsError isEqualToString:@"0"]) {
//    myImageRect= CGRectMake(x,y-((100*riation)/riation),w,h);
//    }else if ([IsError isEqualToString:@"2"])
//    {
//     myImageRect= CGRectMake(x,y+((100*riation)/riation), w,h);
//    }
//    else
...
//    {
//     myImageRect= CGRectMake(x,y-((100*riation)/riation),w,h);
//    }else if ([IsError isEqualToString:@"1"])
//    {
//     myImageRect= CGRectMake(x,y,w,h);
//    }
//    else
